...
function markOcean(
  ...
  coords.forEach((vec) => {
    if (vec[0][1] === vec[1][1]) {
      // horizontal
      return range(vec[0][0], vec[1][0]).forEach((point) =>
        ocean[vec[0][1]][point] += 1
      );
    } else if (vec[0][0] === vec[1][0]) {
      // Vertical
      return range(vec[0][1], vec[1][1]).forEach((point) => {
        ocean[point][vec[0][0]] += 1;
      });
    } else {
      // Diagonal
      const xAxis = range(vec[0][0], vec[1][0]);
      const yAxis = range(vec[0][1], vec[1][1]);
      xAxis.forEach((x, index) => {
        ocean[yAxis[index]][x] += 1;
      });
    }
  });
  return ocean;
